...
function AdminSideNav() {
  const menus = [
    ...
    { name: "Course", link: "/course", icon: AiOutlineBarChart },
    { name: "students", link: "/students", icon: AiOutlineUsergroupAdd , margin: true},
    { name: "Mentors", link: "/mentors", icon: FiMessageSquare , },
  ];
    ...
    <section className=" sm:flex gap-6 hidden  ">
      <div
        className={` ${open ? "w-60 " : "w-20 "
          } bg-gradient-to-b from-sky-800  to-myblue min-h-screen p-5   relative duration-500`}
      >
        <AiFillLeftCircle
          className={`absolute text-white bg-sky-700 rounded-full cursor-pointer -right-4 top-20 w-10 h-10  
             ${!open && "rotate-180"}`}
        ...
        <ul className="pt-16">
          {menus.map((menu, index) => (
            ...
            <NavLink to={menu.link}
              ...
              ` : `flex mt-1 px-4 p-2 cursor-pointer hover:bg-gradient-to-tr hover:from-sky-600 hover:to-myblue hover:text-white rounded-md hover:border-white hover:border  text-gray-300 text-sm2 items-center gap-x-4 
              ` }
            >
